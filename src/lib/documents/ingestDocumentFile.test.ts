/**
 * Tests for SHA-256 deduplication in {@link ingestDocumentFile}.
 *
...
import { computeSha256, ingestDocumentFile } from './ingestDocumentFile.ts';
...
describe('ingestDocumentFile', () => {
  ...
  it('reuses an existing file with matching sha256 and skips scan/upload', async () => {
    ...
    const result = await ingestDocumentFile({
      buffer,
      fileName: 'report.pdf',
