# Type Alias: StringFormatParams\<T, AlsoOmit\>

> **StringFormatParams**\<`T`, `AlsoOmit`\> = [`Params`](Params.md)\<`T`, `NonNullable`\<`T`\[`"_zod"`\]\[`"isst"`\] \| `T`\[`"_zod"`\]\[`"issc"`\]\>, `"type"` \| `"coerce"` \| `"checks"` \| `"error"` \| `"check"` \| `"format"` \| `AlsoOmit`\>
