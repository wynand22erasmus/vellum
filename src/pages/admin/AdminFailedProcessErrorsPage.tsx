/**
 * Admin view of process errors that failed to persist.
 *
...
/** Paginated failed process error table (`/admin/failed-process-errors`). */
export function AdminFailedProcessErrorsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const res = await apiFetch(`/api/admin/failed-process-errors?${params.toString()}`);
      if (cancelled) return;
      if (!res.ok) {
      ...
      setRows(data.failedProcessErrors);
      setTotal(data.total);
      setLoading(false);
    }
  ...
  return (
    <>
      <PageSection description="Dead-letter payloads when process-error enqueue fails.">
        <AdminTableFilters
        ...
        {loading ? (
          ...
          <EmptyState title="No failed process errors match" />
        ) : (
