...
authRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (typeof token !== 'string') {
      throw AppError.badRequest('Missing verification token.');
    }
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (isEmailVerificationSatisfied(user)) {
      ...
      ok(req, res, { verified: true, email: user.email });
      return;
    }
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    okMessage(req, res, 'Verification email sent.');
  }),
);
...
authRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    ok(req, res, { user, provider: env.authProvider });
  }),
);
