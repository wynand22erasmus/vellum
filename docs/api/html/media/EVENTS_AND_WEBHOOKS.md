...
**Process errors:** Operational failures (HTTP 4xx/5xx, worker crashes) are recorded separately in `ProcessError` / `FailedAuditLog`. See [ERROR_HANDLING.md](./ERROR_HANDLING.md). An audit event and a process error may share a `correlationId` (for example verify wrong password).
...
| `timestamp` | Insert time (UTC) |
| `documentId` | Set when the event relates to a document (nullable) |
| `userId` | Set when the event relates to a dashboard user (nullable) |
...
| `FILE_SCRUBBED` | Object removed from storage by lifecycle worker | [`fileScrubWorker`](../src/server/workers/fileScrubWorker.ts) | ✓ | — | `originalS3Key` |
| `LINK_REVOKED` | Download link manually revoked; S3 object deleted | [`revoke-document.ts`](../src/lib/revoke-document.ts) — `POST /api/documents/:id/revoke` | ✓ | optional | `revokedBy`, `authMode` (`admin` \| `api_key`), `fileDeleted: true` |
...
| Verify: expired / revoked / scrubbed / limit / consumed / invalid token | `FILE_DOWNLOAD_FAILED` | [`verify.ts`](../src/server/routes/verify.ts) — use `metadata.reason` |
...
| `FILE_SCRUBBED` | `WEBHOOK_URL_FILE_SCRUBBED` |
...
**Retries:** Failed deliveries (non-2xx, timeout, network error) retry with backoff up to `WEBHOOK_MAX_RETRIES`. Final failures land in `FailedWebhookDelivery` (mirror of `FailedAuditLog`).
